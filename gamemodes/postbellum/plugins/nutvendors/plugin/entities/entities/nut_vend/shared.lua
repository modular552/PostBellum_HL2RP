...
if (SERVER) then
	function ENT:Initialize()
		...
		self:SetDTFloat(1, 9999);
		self:SetDTFloat(2, 9999);
		self:SetDTFloat(3, 9999);
		self:SetDTFloat(4, 9999);
	...
	function ENT:Use(activator)
		...
		if (button and self:GetDTFloat(button) and self:GetDTFloat(button) > 0) then
			local item = "breens_water"
			local price = 5

			if (button == 2) then
				item = "smooth_breens_water"
				price = price + 10
			elseif (button == 3) then
				item = "smooth_breens_water"
				price = price + 15
			elseif (button == 4) then
				item = "citizen_supplements"
				price = price + 25
			end
	...
	function ENT:Draw()
		...
		cam.Start3D2D(position + f*17.33 + r*-19.5 + u*5.75, angles, 0.06)
			draw_SimpleText("Regular", "hl2_MainText", 0, 0, color_white, 0, 0)
			draw_SimpleText("Smooth", "hl2_MainText", 0, 36, color_white, 0, 0)
			draw_SimpleText("Special", "hl2_MainText", 0, 72, color_white, 0, 0)
			draw_SimpleText("Snack", "hl2_MainText", 0, 108, color_white, 0, 0)
		cam.End3D2D()
