...
cwDatastream:Hook("RecogniseMenu", function(data)
	local menuPanel = cwKernel:AddMenuFromData(nil, {
		["Всем персонажам в радиусе шепота."] = function()
			cwDatastream:Start("RecogniseOption", "whisper");
		end,
		["Всем персонажам в радиусе крика."] = function()
			cwDatastream:Start("RecogniseOption", "yell");
		end,
		["Всем персонажам в радиусе разговора."] = function()
			cwDatastream:Start("RecogniseOption", "talk");
		end,
		["Персонажу перед вами."] = function()
...
function Clockwork:GetEntityMenuOptions(entity, options)
	...
	if (class == "cw_item") then
		...
		if (itemTable) then
			local useText = itemTable("useText", "Использовать");
...
local WEAPON_LOWERED_ANGLES = Angle(0, 0, -15);
...
function Clockwork:MenuItemsAdd(menuItems)
	...
	menuItems:Add("Классы", "cwClasses", "Выберите доступный класс из списка.");	--Fix004
	menuItems:Add("Настройки", "cwSettings", "Настройте игровой режим, как вам удобно.");	--Fix004
...
function Clockwork:GetScreenTextInfo()
	...
	if (cwClient:GetSharedVar("CharBanned")) then
		return {
			...
			title = "ВАШ ПЕРСОНАЖ ЗАБАНЕН",		--Fix005
			text = "Перейдите в меню персонажей, чтобы выбрать другого."		--Fix005
		};
	end;
end;
...
function Clockwork:DrawTargetPlayerStatus(target, alpha, x, y)
	...
	if (!target:Alive()) then
		if (target:GetGender() == GENDER_FEMALE) then	--Add
			return self.kernel:DrawInfo(gender.." мертва...", x, y, informationColor, alpha);	--Fix
		else
			return self.kernel:DrawInfo(gender.." мертв...", x, y, informationColor, alpha);	--Add
		end;	--Add
	else	--Add
		return y;
	end;
...
function Clockwork:GetProgressBarInfo()
	...
	if (!self.Client:Alive() and action == "spawn") then
		return {text = "Вы умерли...", percentage = percentage, flash = percentage < 10};		--Fix006
	end;
	
	if (!self.Client:IsRagdolled()) then
		if (action == "lock") then
			return {text = "Закрываем замок...", percentage = percentage, flash = percentage < 10};		--Fix006
		elseif (action == "unlock") then
			return {text = "Открываем замок...", percentage = percentage, flash = percentage < 10};		--Fix006
		end;
	elseif (action == "unragdoll") then
		if (self.Client:GetRagdollState() == RAGDOLL_FALLENOVER) then
			return {text = "Вы встаете...", percentage = percentage, flash = percentage < 10};	--Fix006
		else
			return {text = "Вы приходите в сознание...", percentage = percentage, flash = percentage < 10};	--Fix006
		end;
	elseif (self.Client:GetRagdollState() == RAGDOLL_FALLENOVER) then
		...
		if (fallenOver and self.plugin:Call("PlayerCanGetUp")) then
			return {text = 'Нажмите "Прыжок", чтобы подняться.', percentage = 100};	--Fix006
		end;
	end;
end;
...
function Clockwork:GetPlayerInfoText(playerInfoText)
	...
	if (self.config:Get("cash_enabled"):Get()) then
		if (cash > 0) then
			playerInfoText:Add("CASH", "Жетонов: "..Clockwork.kernel:FormatCash(cash, true));	--Fix007
		end;
...
function Clockwork:GetTargetPlayerText(player, targetPlayerText)
	...
	local physDescTable = {};
	local thirdPerson = "ним";		--Fix008
	
	if (player:GetGender() == GENDER_FEMALE) then
		thirdPerson = "ней";		--Fix008
	end;
	
	if (self.player:DoesRecognise(player, RECOGNISE_PARTIAL)) then
		self.kernel:WrapText(self.player:GetPhysDesc(player), targetIDTextFont, 400, physDescTable); --math.max(ScrW() / 9, 384)
	...
	elseif (player:Alive()) then
		targetPlayerText:Add("PHYSDESC", "Вы незнакомы с "..thirdPerson..".");		--Fix009
	end;
end;
...
function Clockwork:GetPlayerScoreboardText(player)
	...
	if (self.player:DoesRecognise(player, RECOGNISE_PARTIAL)) then
		...
		-- if (string.utf8len(physDesc) and string.utf8len(physDesc) > 190) then		--Fix010
			-- if (string.utf8sub(physDesc,190,190)==" ") then --фикс русских букв	--Add001
				-- return string.utf8sub(physDesc, 1, 189).."...";
			-- else
				-- return string.utf8sub(physDesc, 1, 190).."...";
			-- end;	--Add001	
		-- else
			return physDesc;
		-- end;
	else
		return "Вы незнакомы с "..thirdPerson..".";	--Fix
	end;
end;
...
function Clockwork:GetPlayerScoreboardOptions(player, options, menu)
	...
	if (charBan and self.player:HasFlags(self.Client, charBan.access)) then
		options["Бан персонажа"] = function()	--Fix
			RunConsoleCommand("cwCmd", "CharBan", player:Name());
		end;
	end;
	
	if (plyKick and self.player:HasFlags(self.Client, plyKick.access)) then
		options["Кик игрока"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Укажите причину кика.", nil, function(text)	--Fix
	...
	if (plyBan and self.player:HasFlags(self.Client, self.command:FindByID("PlyBan").access)) then
		options["Бан игрока"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Укажите время бана в минутах.", nil, function(minutes)	--Fix
				Derma_StringRequest(player:Name(), "Укажите причину бана", nil, function(reason)	--Fix
	...
	if (charGiveFlags and self.player:HasFlags(self.Client, charGiveFlags.access)) then
		options["Выдать Флаг"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Укажите флаги, которые нужно добавить игроку.", nil, function(text)	--Fix
	...
	if (charTakeFlags and self.player:HasFlags(self.Client,charTakeFlags.access)) then
		options["Забрать Флаг"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Укажите флаги, которые нужно забрать. b - флаг должен быть всегда!", player:GetSharedVar("Flags"), function(text)	--Fix
	...
	if (charSetName and self.player:HasFlags(self.Client, charSetName.access)) then
		options["Установить имя"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Введите имя.", player:Name(), function(text)	--Fix
	...
	if (charGiveItem and self.player:HasFlags(self.Client, charGiveItem.access)) then
		options["Выдать предмет"] = function()	--Fix
			Derma_StringRequest(player:Name(), "Укажите предмет.", nil, function(text)	--Fix
	...
	if (plySetGroup and self.player:HasFlags(self.Client, plySetGroup.access)) then
		options["Установить группу"] = {};	--Fix
		options["Установить группу"]["Super Admin"] = function()	--Fix
			Clockwork.kernel:RunCommand("PlySetGroup", player:Name(), "superadmin");
		end;
		options["Установить группу"]["Admin"] = function()	--Fix
			Clockwork.kernel:RunCommand("PlySetGroup", player:Name(), "admin");
		end;
		options["Установить группу"]["Operator"] = function()	--Fix
...
concommand.Add("cwLua", function(player, command, arguments)
	...
	print("У вас нет доступа к этой команде, "..player:Name()..".");
end);
