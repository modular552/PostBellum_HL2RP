...
function PANEL:Init()
	...
	self.textEntry = self.settingsForm:TextEntry("Текст на двери.");
	...
	if (Clockwork.door:GetOwner() == Clockwork.Client) then
		...
		if (!Clockwork.door:IsUnsellable()) then
			...
			if (doorCost > 0) then
				button = self.settingsForm:Button("Продать");
			else
				button = self.settingsForm:Button("Отказаться от владения.");
			end;
			
			-- Called when the button is clicked.
			function button.DoClick(button)
				if (doorCost > 0) then
					Derma_Query("Вы действительно хотите продать эту дверь?", "Продать дверь.", "Да", function()
					...
					Derma_Query("Вы действительно хотите перестать быть владельцем этой двери?", "Владение дверью.", "Да", function()
						Clockwork.datastream:Start("DoorManagement", {Clockwork.door:GetEntity(), "Sell"});
	...
	self.propertySheet:AddSheet("Игроки", self.playersPanel, "icon16/user.png", nil, nil, "Доступ к двери.");
	self.propertySheet:AddSheet("Настройки", self.settingsPanel, "icon16/wrench.png", nil, nil, "Настройки двери.");
...
function PANEL:Rebuild()
	...
	if (table.Count(categories) > 0) then
		for k, v in pairs(categories) do
			...
			if (k == 1) then
				collapsibleCategory:SetLabel("Персонажи с полным доступом.");
				collapsibleCategory:SetCookieName("cwDoorComplete");
			elseif (k == 2) then
				collapsibleCategory:SetLabel("Персонажи с базовым доступом.");
				collapsibleCategory:SetCookieName("cwDoorBasic");
			else
				collapsibleCategory:SetLabel("Персонажи без доступа.");
...
Clockwork.datastream:Hook("PurchaseDoor", function(data)
	...
	if (doorCost > 0) then
		Derma_Query("Вы действительно хотите купить эту дверь за "..Clockwork.kernel:FormatCash(Clockwork.config:Get("door_cost"):Get(), nil, true).."?", "Покупка двери.", "Да", function()
		...
		Derma_Query("Вы действительно хотите стать владельцем этой двери?", "Владение дверью.", "Да", function()
			Clockwork.datastream:Start("DoorManagement", {data, "Purchase"});
