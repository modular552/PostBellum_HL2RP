		...
		if (table.Count(permits) > 0) then
			...
			if (Clockwork.player:HasFlags(Clockwork.Client, "x")) then
				...
				panel.customData = {
					...
					name = "Create "..Clockwork.option:GetKey("name_business")
				};
...
function Schema:GetEntityMenuOptions(entity, options)
	...
	elseif (entity:GetClass() == "cw_breach") then
		options["Активировать"] = "cw_breachCharge";
	elseif (entity:GetClass() == "cw_radio") then
		...
		options["Установить частоту"] = function()
			Derma_StringRequest("Частота", "Какую радиочастоту вы хотите установить?", frequency, function(text)
				if (IsValid(entity)) then
					Clockwork.entity:ForceMenuOption(entity, "Установить частоту", text);
				end;
			end);
		end;
...
function Schema:GetCinematicIntroInfo()
	return {
		credits = "Разработчики сервера TIMON_Z1535 и -=dimidrol=-",
...
function Schema:GetPlayerScoreboardClass(player)
	...
	elseif (faction == FACTION_REFUGEE) then
		return "Беженцы";
	elseif (faction == FACTION_RAIDER) then
		return "Рейдеры";
	elseif (faction == FACTION_CWU or faction == FACTION_CWUMEDIC or faction == FACTION_CWUBOSS) then
		return "Гражданский Союз Рабочих";
	elseif (faction == FACTION_REBEL or faction == FACTION_REBELMEDIC or faction == FACTION_REBELBOSS) then
		return "Сопротивление";
	elseif (faction == FACTION_SYNDICATE) then
		return "Синдикат";
	elseif (faction == FACTION_VORT or faction == FACTION_VORTSLAVE) then
		return "Вортигонты";
	elseif (faction == FACTION_MPF) then
		return "Гражданская Оборона";
	elseif (faction == FACTION_GOO) then
		return "Городское Ополчение";
	elseif (faction == FACTION_ADMIN) then
		return "Городской администратор";
	elseif (faction == FACTION_CREMATOR) then
		return "Синтеты";
	elseif (faction == FACTION_OTA) then
		return "Сверхчеловеческий Отдел";
	end;
end;
...
function Schema:PlayerCanSeeClass(class)
	if (class.index == CLASS_MPS and !self:IsPlayerCombineRank(Clockwork.Client, "SCN")) then
		return false;
	elseif (class.index == CLASS_MPR and !self:IsPlayerCombineRank(Clockwork.Client, "RCT")) then
		return false;
	elseif (class.index == CLASS_EMP and !self:IsPlayerCombineRank(Clockwork.Client, "EpU")) then
		return false;
	elseif (class.index == CLASS_OWS and !self:IsPlayerCombineRank(Clockwork.Client, "OWS")) then
		return false;
	elseif (class.index == CLASS_SOW and !self:IsPlayerCombineRank(Clockwork.Client, "SOW")) then
		return false;
	elseif (class.index == CLASS_EOW and !self:IsPlayerCombineRank(Clockwork.Client, "EOW")) then
		return false;
	elseif (class.index == CLASS_MPU) then
		if (self:IsPlayerCombineRank(Clockwork.Client, "SCN") or self:IsPlayerCombineRank(Clockwork.Client, "EpU")
		or self:IsPlayerCombineRank(Clockwork.Client, "RCT")) then
			return false;
		end;
	end;
end;
...
function Schema:DrawTargetPlayerStatus(target, alpha, x, y)
	...
	if (target:Alive()) then
		...
		if (target:GetRagdollState() == RAGDOLL_KNOCKEDOUT) then
			mainStatus = gender.." явно без сознания.";
		end;
		
		if (target:GetSharedVar("tied") != 0) then
			if (Clockwork.player:GetAction(Clockwork.Client) == "untie") then
				if (target:GetGender() == GENDER_FEMALE) then
					mainStatus = gender.." будет освобождена.";
				else
					mainStatus = gender.. " будет освобожден.";
				end;
			else
				local untieText;
				...
				if (target:GetShootPos():Distance( Clockwork.Client:GetShootPos() ) <= 192) then
					if (Clockwork.Client:GetSharedVar("tied") == 0) then
						mainStatus = "Нажмите :+use: чтобы освободить "..thirdPerson..".";
		...
		elseif (Clockwork.player:GetAction(Clockwork.Client) == "tie") then
			if (target:GetGender() == GENDER_FEMALE) then
				mainStatus = gender.." будет связана.";
			else
				mainStatus = gender.." будет связан.";
			end;
		end;
...
function Schema:GetPlayerInfoText(playerInfoText)
	...
	if (citizenID) then
		if (Clockwork.Client:IsCitizen()) then
			playerInfoText:Add("CITIZEN_ID", "Персональный ID: #"..citizenID);
		end;
	end;
end;
			...
			if (!self.nextHealthWarning or curTime >= self.nextHealthWarning) then
				if (self.lastHealth) then
					if (health < self.lastHealth) then
						if (health == 0) then
							self:AddCombineDisplayLine( "СИСТњМЫ ЖИЗНЕОќЕСПЕЧЕНИЯ ВћШЛИ ИЗ СТРО&! Авар&µное выключєнЃЂ ±ђ¶¬џ ќРќРћРЎ###", Color(255, 0, 0, 255) );
						else
							self:AddCombineDisplayLine( "ОПАСНОСТЬ! Обнаружены телесные повреждения...", Color(255, 0, 0, 255) );
						end;
						
						self.nextHealthWarning = curTime + 2;
					elseif (health > self.lastHealth) then
						if (health == 100) then
							self:AddCombineDisplayLine( "Внутренние системы организма восстановлены!", Color(0, 255, 0, 255) );
						else
							self:AddCombineDisplayLine( "Внутренние системы организма восстанавливаются...", Color(0, 0, 255, 255) );
						end;
				...
				if (self.lastArmor) then
					if (armor < self.lastArmor) then
						if (armor == 0) then
							self:AddCombineDisplayLine( "ОПАСНОСТЬ! Внешняя защита исчерпана...", Color(255, 0, 0, 255) );
						else
					...
					elseif (armor > self.lastArmor) then
						if (armor == 100) then
							self:AddCombineDisplayLine( "Внешние системы защиты восстановлены!", Color(0, 255, 0, 255) );
						else
							self:AddCombineDisplayLine( "Внешние системы защиты восстанавливаются...", Color(0, 0, 255, 255) );
						end;
