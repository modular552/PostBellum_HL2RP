...
function PANEL:Rebuild()
	...
	if (table.Count(classes) > 0) then
		...
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Нет игроков для отображения.");
...
function PANEL:Init()
	SCOREBOARD_PANEL = true;
		...
		if (info.avatarImage) then
			self.avatarButton:SetToolTip("Ник игрока: "..info.steamName..".\nSteamID игрока: "..info.player:SteamID()..".");
...
function PANEL:Think()
	if (IsValid(self.player)) then
		if (self.toolTip) then
			...
			self.spawnIcon:SetToolTip("Пинг игрока: "..self.player:Ping()..".");
		end;
	end;
