...
function cwAnimatedLegs:ShouldDrawLegs()
	return IsValid(self.LegsEntity) and Clockwork.Client:Alive() and !Clockwork.Client:InVehicle() and GetViewEntity() == Clockwork.Client and !Clockwork.Client:ShouldDrawLocalPlayer() and !Clockwork.Client:IsNoClipping() and !Clockwork.Client:IsRagdolled() and !Schema:GetScannerEntity(Clockwork.Client)
end;
...
function cwAnimatedLegs:CreateLegs()
	...
	timer.Simple(1, function()
		if (self and self.LegsEntity) then
			local id = self.LegsEntity:LookupBone("ValveBiped.Bip01_Spine2")
			
			if (!id) then
				id = self.LegsEntity:LookupBone("ValveBiped.spine2")
			end
			
			if (id) then
				self:ApplyChildRecursive(self.LegsEntity, id)
			end
		end
...
function cwAnimatedLegs:ApplyChildRecursive(ent, id, restore)
	if (restore) then
		for _, v in pairs(ent:GetChildBones(id)) do
			ent:ManipulateBoneScale(v, Vector(1, 1, 1))
			ent:ManipulateBonePosition(v, Vector(0, 0, 0))
			self:ApplyChildRecursive(ent, v, true)
		end
	else
		for _, v in pairs(ent:GetChildBones(id)) do
			ent:ManipulateBoneScale(v, Vector(0, 0, 0))
			ent:ManipulateBonePosition(v, Vector(-10, -10, 0))
			self:ApplyChildRecursive(ent, v)
		end
	end
end
	...
	if (IsValid(self.LegsEntity)) then
		...
		if (self.LegsEntity:GetModel() != Clockwork.Client:GetModel()) then
			...
			timer.Simple(0.5, function()
				self:ApplyChildRecursive(self.LegsEntity, 0, true)
