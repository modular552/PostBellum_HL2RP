...
function ITEM:OnHandleUnequip(Callback)
	if (self.OnDrop) then
		local menu = DermaMenu();
			...
			menu:AddOption("Разоружить", function()
				Callback();
			end);
	...
	if (itemTable:IsTheSameAs(self)) then
		...
		elseif (Clockwork.plugin:Call("PlayerCanDropWeapon", player, self, weapon)) then
			...
			if (player:GetShootPos():Distance(trace.HitPos) <= 192) then
				...
				Clockwork.player:Notify(player, "Вы не можете выбросить оружие так далеко!");
			end;
		end;
	end;
end;
...
function ITEM:OnUse(player, itemEntity)
	if (self("whitelist") and !table.HasValue(self("whitelist"), player:GetFaction())) then
		Clockwork.player:Notify(player, "Вы не можете снарядить это оружие!");
		return false;
	end;
...
if (CLIENT) then
	function ITEM:GetClientSideInfo()
		...
		if (clipOne > 0) then
			clientSideInfo = Clockwork.kernel:AddMarkupLine(clientSideInfo, "В обойме: "..clipOne);
		end;
		
		if (clipTwo > 0) then
			clientSideInfo = Clockwork.kernel:AddMarkupLine(clientSideInfo, "Подствольник: "..clipTwo);
		end;
