...
if (SERVER) then
	...
	function Clockwork.kernel:DrawDateTime()
		...
		if (Clockwork.LastDateTimeInfo and Clockwork.LastDateTimeInfo.y > info.y) then
			...
			self:DrawInfo("ИНФОРМАЦИЯ О ПЕРСОНАЖЕ", x, y + 4, colorInfo, nil, true, function(x, y, width, height)	--Fix
				return x, y - height;
			end);
			...
			if (self:CanCreateInfoMenuPanel() and self:IsInfoMenuOpen()) then
				...
				self:DrawInfo("БЫСТРОЕ МЕНЮ", x, y, colorInfo, nil, true, function(x, y, width, height)	--Fix
		...
		if (Clockwork.plugin:Call("PlayerCanSeeDateTime")) then
			...
			if (dateString and timeString) then
				...
				local text = dateString..". "..dayName..", "..timeString..".";
	...
	function Clockwork.kernel:SetRecogniseMenu(menuPanel)
		...
		self:SetTitledMenu(menuPanel, "ПОКАЗАТЬ КАК ВАС ЗОВУТ");
	end;
	...
	function Clockwork.kernel:HandleItemSpawnIconClick(itemTable, spawnIcon, Callback)
		...
		for k, v in pairs(itemFunctions) do
			...
			local dropText = itemTable("dropText", "Выбросить");
			local destroyText = itemTable("destroyText", "Уничтожить");
			
			if ((!useText and v == "Использовать") or (useText and v == useText)) then
			...
			elseif ((!dropText and v == "Выбросить") or (dropText and v == dropText)) then
				itemMenu:AddOption(v, function()
					if (itemTable) then
						self:RunCommand(
			...
			elseif ((!destroyText and v == "Уничтожить") or (destroyText and v == destroyText)) then
				local subMenu = itemMenu:AddSubMenu(v);
				
				subMenu:AddOption("Да", function()
					if (itemTable) then
						self:RunCommand(
				...
				subMenu:AddOption("Нет", function() end);
			elseif (type(v) == "table") then
	...
	function Clockwork.kernel:HandleItemSpawnIconRightClick(itemTable, spawnIcon)
		if (itemTable.OnHandleRightClick) then
			...
			if (functionName and functionName != "Использовать") then
				local customFunctions = itemTable("customFunctions");
