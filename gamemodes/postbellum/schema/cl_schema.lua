...
Schema.randomDisplayLines = {
	"Новые диррективы получены...",
	"Удаление устаревших данных...",
	"Фильтрация данных...",
	"Устранение конфликтующих данных...",
	"Обновление координат биосигнала...",
	"Протоколирование состояния системы...",
	"Просто троеточие...",
	"Новые уведомления отсутствуют...",
	"Проверка связи с центром...",
	"Синхронизация данных о местоположении...",
	"Интерпретация радиосообщений...",
	"Бездействие...",
	"Сканирование местности...",
	"Очистка журнала активности...",
	"Ты правда это читаешь? В нас пропал дух авантюризма...",
	"Диспетчер сообщает, что пролил кофе...",
	"Диспетчер начал разлагаться в Нексусе...",
	"Ожидание соединения..."
};

Clockwork.directory:AddCategory("Диспетчер", "Commands");
Clockwork.directory:AddCategory("Гражданская Оборона", "Commands");
...
Clockwork.datastream:Hook("Frequency", function(data)
	Derma_StringRequest("Частота", "Какую радиочастоту вы хотите установить?", data, function(text)
...
Clockwork.chatBox:RegisterClass("request_eavesdrop", "ic", function(info)
	if (info.shouldHear) then
		Clockwork.chatBox:Add(info.filtered, nil, Color(255, 255, 150, 255), info.name.." запрашивает \""..info.text.."\"");
	end;
end);

Clockwork.chatBox:RegisterClass("broadcast", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 125, 175, 255), info.name.." сообщает \""..info.text.."\"");
end);

Clockwork.chatBox:RegisterClass("dispatch", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 100, 100, 255), "Диспетчер сообщает \""..info.text.."\"");
end);

Clockwork.chatBox:RegisterClass("request", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(175, 125, 100, 255), info.name.." запрашивает \""..info.text.."\"");
end);
