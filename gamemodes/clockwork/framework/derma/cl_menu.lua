...
function PANEL:Init()
	if (!Clockwork.theme:Call("PreMainMenuInit", self)) then
		...
		self.closeMenuLabel:SetText("ЗАКРЫТЬ МЕНЮ");
		self.closeMenuLabel:SetCallback(function(button)
			self:SetOpen(false);
		end);
		self.closeMenuLabel:SetToolTip("Нажмите, чтобы вернуться к игре.");
		...
		self.characterMenuLabel:SetText("ПЕРСОНАЖИ");
		self.characterMenuLabel:SetCallback(function(button)
			self:SetOpen(false);
			Clockwork.character:SetPanelOpen(true);
		end);
		self.characterMenuLabel:SetToolTip("Нажмите, чтобы открыть меню персонажей.");
