...
function PANEL:Init()
	if (!Clockwork.theme:Call("PreCharacterMenuInit", self)) then
		...
		self.authorLabel:SetText(Schema:GetAuthor());
		...
		self.createButton:SetCallback(function(panel)
			if (table.Count(Clockwork.character:GetAll()) >= Clockwork.player:GetMaximumCharacters()) then
				return Clockwork.character:SetFault("Вы больше не можете создавать персонажей!");
			end;
...
function PANEL:Init()
	...
	self.useButton:SetToolTip("Выбрать этого персонажа.");
	...
	self.deleteButton:SetToolTip("Удалить этого персонажа.");
	...
	function modelPanel.DoClick(modelPanel)
		...
		if (activePanel:GetSelectedModel() == self) then
			...
			options["Удалить"] = {};
			options["Удалить"]["Нет"] = function() end;
			options["Удалить"]["Да"] = function()
				Clockwork.datastream:Start("InteractCharacter", {
					characterID = self.customData.characterID, action = "delete"}
				);
			end;
...
function PANEL:Think()
	...
	local details = Clockwork.kernel:MarkupTextWithColor("Описание", informationColor);
	
	details = Clockwork.kernel:AddMarkupLine(
		details, self.customData.details or "Пусто."
	);
	
	--if (toolTip and toolTip != "") then
	--	details = Clockwork.kernel:AddMarkupLine(details, "Информация", informationColor);
...
function PANEL:Init()
	...
	self.helpText = self.attributesForm:Help("Вы можете потратить еще "..maximumPoints.." очков.");
...
function PANEL:Think()
	...
	if (self.helpText) then
		...
		self.helpText:SetText("Вы можете потратить еще "..pointsLeft.." очков.");
	end;
...
function PANEL:Init()
	...
	self.classesForm:SetName("Класс");
	...
	for k, v in pairs(Clockwork.class:GetAll()) do
		if (v.isOnCharScreen and (v.factions and table.HasValue(v.factions, self.info.faction))) then
			self.classTable = v;
			self.overrideData = {
				information = "Выберите класс вашего персонажа.",
				Callback = function()
...
function PANEL:OnNext()
	if (!self.info.class or !Clockwork.class:FindByID(self.info.class)) then
		Clockwork.character:SetFault("Вы не выбрали класс!");
		return false;
	end;
end;
...
function PANEL:Init()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		...
		self.nameForm:SetName("Имя персонажа");
	...
	if (self.bSelectModel or self.bPhysDesc) then
		...
		self.appearanceForm:SetName("Внешность персонажа");
		
		if (self.bPhysDesc and self.bSelectModel) then
			self.appearanceForm:Help("Опишите внешний вид вашего персонажа, и выберите подходящую модель.");
		elseif (self.bPhysDesc) then
			self.appearanceForm:Help("Опишите внешний вид вашего персонажа.");
		end;
...
function PANEL:OnNext()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		if (IsValid(self.fullNameTextEntry)) then
			...
			if (self.info.fullName == "") then
				Clockwork.character:SetFault("Пустое имя не допускается!");
				return false;
			end;
			
			local fullfind = select(3, string.find(self.info.fullName, "([%a']+)"))
			
			if (fullfind != self.info.fullName) then
				Clockwork.character:SetFault("Ваше имя может содержать только буквы английского алфавита и апостроф (')!");
				return false;
			end;
			
			if (string.utf8len(self.info.fullName) < 2) then
				Clockwork.character:SetFault("Ваше имя должно быть длиной не менее 2-х символов!");
				return false;
			end;
			
			if (string.utf8len(self.info.fullName) > 16) then
				Clockwork.character:SetFault("Ваше имя должно быть длиной не более 16-ти символов!");
				return false;
			end;
		else
			...
			local forefind = select(3, string.find(self.info.forename, "([абвгдеёжзийклмнопрстуфхцчшщъьыэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ%a]+)"))
			local surfind = select(3, string.find(self.info.surname, "([абвгдеёжзийклмнопрстуфхцчшщъьыэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ%a]+)"))
			...
			if (string.utf8len(self.info.forename) < 2 or string.utf8len(self.info.surname) < 2) then
				Clockwork.character:SetFault("Ваши имя и фамилия должны быть длиной не менее 2-х символов!");
				return false;
			end;
			
			if (string.utf8len(self.info.forename) > 16 or string.utf8len(self.info.surname) > 16) then
				Clockwork.character:SetFault("Ваши имя и фамилия должны быть длиной не более 16-ти символов!");
				return false;
			end;
		end;
	end;
	...
	if (self.bSelectModel and !self.info.model) then
		Clockwork.character:SetFault("Вы не выбрали модель!");
		return false;
	end;
	...
	if (self.bPhysDesc) then
		...
		if (!string.utf8len(self.info.physDesc) or string.utf8len(self.info.physDesc) < minimumPhysDesc) then
			Clockwork.character:SetFault("Описание вашего персонажа должно быть не менее "..minimumPhysDesc.."-х символов!");
			return false;
		end;
	end;
end;
...
function PANEL:Init()
	...
	if (#factions > 1) then
		self.settingsForm:Help("Фракция определяет общий характер персонажа.");
		self.factionMultiChoice = self.settingsForm:ComboBox("Фракция");
			...
			for k, v in pairs(Clockwork.faction.stored) do
				if (v.name == value) then
					if (IsValid(self.genderMultiChoice)) then
						...
						self.genderMultiChoice = self.settingsForm:ComboBox("Пол");
						self.settingsForm:Rebuild();
					end;
					
					if (v.singleGender) then
						self.genderMultiChoice:AddChoice("Не определен", "Male");
					else
						self.genderMultiChoice:AddChoice("Женщина", "Female");
						self.genderMultiChoice:AddChoice("Мужчина", "Male");
					end;
...
function PANEL:OnNext()
	if (IsValid(self.genderMultiChoice)) then
		...
		local gender = self.genderMultiChoice:GetOptionData( self.genderMultiChoice:GetSelectedID() ) or "Male";
	...
	Clockwork.character:SetFault("Вы не выбрали фракцию!");
	return false;
end;
...
Clockwork.character:RegisterCreationPanel("Фракция", "cwCharacterStageOne");
Clockwork.character:RegisterCreationPanel("Описание", "cwCharacterStageTwo");

Clockwork.character:RegisterCreationPanel("Класс", "cwCharacterStageThree",
	function(info)
